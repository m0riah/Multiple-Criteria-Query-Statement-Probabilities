...
/**
 * Calculate the Query Command and Table Names (QCTN) based Query Probabilities using Markov chain mathematical model.
 *
 * @author sky
 * @version 2/17/14
 */
@Service("QctnBasedQueryProbabilityCalculationService")
public class QctnBasedQueryProbabilityCalculationService implements QueryProbabilityCalculationService {
    ...
    public boolean updateQueryProbabilitiesForAllUser() {
        ...
        // Process through all the users in the QCTN based user table.
        while (userItr.hasNext()) {
            ...
            String tptiTableName = transitionTableNameIndexDao.generateTableName(user.getUserName());
            // Get a Collection of the QCTN based Query Transition Table names related to a specific user from
            // the Query Transition Table Name Index table.
            Collection<TransitionTableNameIndex> transitionTableNameIndexes =
                    transitionTableNameIndexDao.getAllTransitionTableIndexes(tptiTableName);
            Iterator<TransitionTableNameIndex> queryPatternIterator =
                    transitionTableNameIndexes.iterator();
            int i = 0;
            while (queryPatternIterator.hasNext()) {
                TransitionTableNameIndex transitionTableIndex = queryPatternIterator.next();
     ...
     * <p/>
     * delta = (1 - VALUE_P) / (total number of queries executed by the user), which is the probability of a
        ...
        while (queryIdIterator.hasNext()) {
            ...
            if (queryTransitionProbabilities.size() == 0) {
                ...
                while (transitionProbabilityIterator.hasNext()) {
                    ...
                    // find a row number of the selected transient transition pattern.
     ...
     * the Query Probability Table.
     * <p>
     *     For mitigation, if the iteration is over 100, stop the calculation.
     * </p>
     *
     * @param patternTransitionProbabilityMatrix the Pattern Transition PROBABILITY Matrix.
     * @return the steady-state vector (ideally).
     */
    public double[] applyMarkovChain(double[][] patternTransitionProbabilityMatrix) {

        int matrixLength = patternTransitionProbabilityMatrix.length;
        ...
        // This means that the probabilities to transit to different patterns are all the same.
        ...
        while (true) {
            ...
            for (int i = 0; i < matrixLength; i++) {
                for (int j = 0; j < matrixLength; j++) {
                    resultVector[i] += patternTransitionProbabilityMatrix[i][j] * previousVector[j];
    ...
    /**
     * Update the Query Probability Table for the user.
     *
