...
/**
 * Query Command and Table Names (QCTN) based Query Transition Table Name Index table data access object.
 *
 * @author sky
 * @version 2/13/14
 */
@Repository("QctnBasedTransitionTableNameIndexDao")
public class QctnBasedTransitionTableNameIndexDao implements TransitionTableNameIndexDao {

    /**
     * Table base name.
     */
    private static final String TABLE_BASE_NAME = "_qctn_transition_index";
    ...
    /**
     * Predefined select all query base.
     */
    ...
    public Collection<TransitionTableNameIndex> getAllTransitionTableIndexes(String tableName) {
        ...
        Collection<TransitionTableNameIndex> transitionTableIndexes = new ArrayList<>();
        for (Row row : results) {
            TransitionTableNameIndex transitionTableNameIndex = new TransitionTableNameIndex();
            transitionTableNameIndex.setQueryId(row.getString(QUERY_ID));
            transitionTableNameIndex.setTransitionTableName(row.getString(QUERY_TRANSITION_TABLE_NAME));
            transitionTableIndexes.add(transitionTableNameIndex);
        }
        return transitionTableIndexes;
    }
